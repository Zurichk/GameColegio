...
struct Connect4Session {
    ...
    turn: char, // 'R' rojo (jugador), 'Y' amarillo (CPU)
    winner: Option<char>,
    draw: bool,
}
...
impl Connect4Session {
    fn new() -> Self {
        Self { board: [[None; COLS]; ROWS], turn: 'R', winner: None, draw: false }
    }
...
fn update_connect4(
    ...
    if restart_clicks.single().map_or(false, |i| *i == Interaction::Pressed) { *session = Connect4Session::new(); }
    if session.winner.is_none() && !session.draw {
        ...
        if let Some(col) = clicked {
            if session.board[0][col].is_none() {
                ...
                // CPU juega si aún no hay ganador y es su turno
                if session.winner.is_none() && !session.draw && session.turn == 'Y' {
                    // pequeño delay simulado vía turno inmediato
                    session.cpu_move();
                }
            }
        }
    ...
    for (field, mut text) in &mut status_text {
        if field.0 == Connect4Field::Status {
            if let Some(w) = session.winner {
                ...
                *text = Text::new(if session.turn == 'R' { "Turno: Rojo" } else { "Turno: Amarillo (CPU)" }.to_string());
            }
        }
    }
