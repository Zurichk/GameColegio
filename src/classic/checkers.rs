...
fn update_checkers(
    ...
    mut cell_texts: Query<&mut Text, Without<CheckersText>>,
    ...
    if restart_clicks.single().map_or(false, |i| *i == Interaction::Pressed) { *session = CheckersSession::new(); }
    ...
    for (field, mut text) in &mut texts {
        if field.0 == CheckersField::Status {
            *text = Text::new(format!("Turno: {} {}", if session.turn==Piece::Red {"Rojas (tú)"} else {"Negras (CPU)"}, session.selected.map(|(r,c)| format!("  Seleccionada: {},{}", r+1, c+1)).unwrap_or("".to_string())));
        }
    }
        ...
        *bg = BackgroundColor(if is_selected { Color::srgb(0.30, 0.60, 0.30) } else { base });
        for child in children.iter() {
            if let Ok(mut text) = cell_texts.get_mut(child) {
                *text = Text::new(match session.board[r][c] {
                    Piece::Empty => "".to_string(),
                    Piece::Red => "●".to_string(),
                    Piece::Black => "○".to_string(),
                    Piece::RedKing => "♔".to_string(),
                    Piece::BlackKing => "♚".to_string(),
                });
                // color del texto según pieza
                // (no cambiamos TextColor por simplicidad, usamos blanco)
            }
        }
    }
}
